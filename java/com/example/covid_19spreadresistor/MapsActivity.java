               ...
               reference.addValueEventListener(new ValueEventListener() {
                   ...
                   public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                       ...
                       while (it.hasNext()) {
                           ...
                           if (Double.parseDouble(id.child("shield strength").getValue().toString()) >= 200) {
                               LatLng l = new LatLng(Double.parseDouble(id.child("lat").getValue().toString()), Double.parseDouble(id.child("lon").getValue().toString()));
                               mMap.addMarker(new MarkerOptions().position(l).title(id.getKey()).icon(BitmapDescriptorFactory.defaultMarker(
                                       BitmapDescriptorFactory.HUE_GREEN)));
                               mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(l, 16.0f));

                           } else if (Double.parseDouble(id.child("shield strength").getValue().toString()) >= 100 &&
                                   Double.parseDouble(id.child("shield strength").getValue().toString()) < 200) {
                               LatLng l = new LatLng(Double.parseDouble(id.child("lat").getValue().toString()), Double.parseDouble(id.child("lon").getValue().toString()));
                               mMap.addMarker(new MarkerOptions().position(l).title(id.getKey()).icon(
